...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function createCardElement(blog) {
    // Build full PDF URL including basePath
    const fullPdfPath = `${basePath}/${blog.pdf}`;
    ...
    li.innerHTML = `
      <div class="card-wrapper">
        ...
        <div class="card-content">
          ...
          <a href="blog.html?file=${encodeURIComponent(fullPdfPath)}" class="button w-button card-link" aria-label="Read more about ${blog.title}">Read More</a>
        </div>
      </div>
    `;
    ...
    li.addEventListener('click', (e) => {
      if (e.target.tagName.toLowerCase() !== 'a' && !e.target.closest('a')) {
        window.location.href = `blog.html?file=${encodeURIComponent(fullPdfPath)}`;
      }
    });
